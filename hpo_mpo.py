...
The ``num_action_samples`` factor accounts for compute cost: 30 action
samples take roughly 3× as long per step as 10.  This prevents the
search from gratuitously choosing high sample counts that increase
wall-clock time without a matching reward gain.

Intermediate eval results are reported to Optuna for pruning
(MedianPruner), so trials that lag behind are killed early.
...
def build_parser():
    ...
    p.add_argument('--step_penalty', type=float, default=5.0,
                   help='Penalty per 1000 steps (scaled by num_action_samples/10) '
                        'subtracted from objective (encourages fewer steps and '
                        'fewer action samples)')
...
def make_objective(args):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        hp = {
            ...
            'dual_lr': trial.suggest_float('dual_lr', 1e-3, 5e-2, log=True),
            'num_action_samples': trial.suggest_categorical('num_action_samples', [10, 20, 30]),
            'eps_eta': trial.suggest_float('eps_eta', 0.05, 0.2),
            'eps_mu': trial.suggest_float('eps_mu', 1e-4, 1e-2, log=True),
            'eps_sigma': trial.suggest_float('eps_sigma', 1e-7, 1e-5, log=True),
        ...
        # Compute-cost factor: num_action_samples scales wall-clock time
        # linearly (10 → 1×, 20 → 2×, 30 → 3×).  The penalty is multiplied
        # by this factor so the optimiser cannot "cheat" by using more
        # samples without paying the compute cost.
        nas = hp['num_action_samples']
        cost_factor = nas / 10.0
        ...
        print(f"Trial {trial.number} | steps={trial_steps} | eval_every={args.eval_every} | "
              f"step_penalty={args.step_penalty} | cost_factor={cost_factor:.1f} "
              f"(nas={nas}) | params: {hp}")
        print(f"{'='*60}")
        ...
        # Penalised objective: reward minus compute cost
        ...
        trial.set_user_attr('num_action_samples', nas)
