"""
MAPPO Dynamic Scoring Agent testen und visualisieren.

Testet einen mit Dynamic Scoring Rewards trainierten MAPPO-Agenten.

Usage:
    python test_mappo_dynamic.py                          # Neuesten Checkpoint laden
    python test_mappo_dynamic.py --checkpoint path.pt     # Spezifischen Checkpoint laden
    python test_mappo_dynamic.py --num-episodes 5         # Mehrere Episoden

Beispiele:
    # Lokal trainierten Agenten testen
    python test_mappo_dynamic.py --checkpoint logs/optuna/trial_16/best_agent.pt --viewer

    # Besten Checkpoint aus Optuna-Study laden
    python test_mappo_dynamic.py --log-dir logs/optuna/trial_10
"""
import argparse
import os
import glob
...
def find_latest_checkpoint(log_dir="logs/soccer_mappo_dynamic"):
...
def test(args):
    ...
    print("\nDynamic Scoring Agent - trained with:")
    print("  - Recovery: Ball zurueckerobern")
    print("  - Pursuit:  Ball zum Tor bringen")
    print("  - Possession: Ball kontrollieren")
    print("  - Defense:  Tor verteidigen")
    
    try:
        viewer.launch(env, policy=policy, title="MAPPO Dynamic Scoring Soccer Agent")
    except Exception as e:
        print(f"Viewer error: {e}")
        print("Make sure you have a display available.")
        print("\nAlternative: Run without viewer to just load and verify the model:")
        print(f"  python test_mappo_dynamic.py --checkpoint {args.checkpoint}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test MAPPO Dynamic Scoring Agent",
