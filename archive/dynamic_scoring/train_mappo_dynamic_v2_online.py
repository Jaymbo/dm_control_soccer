"""
Online MAPPO mit Dynamic Scoring V2 für DM Control Soccer.

Im Gegensatz zum batch-basierten train_mappo_dynamic_v2.py:
- Parallele Environments (num_envs) sammeln Daten gleichzeitig
- Update nach jedem Rollout (rollout_steps) statt nach 10 Episoden
- N-Step Returns für schnelleres Feedback
- Beibehaltung der MAPPO-Architektur (shared actor + zentraler critic)

Ziel: Minimale Latenz zwischen Aktion -> Reward -> Policy-Update ("heiß/kalt"-Lernen).
"""
import os
import argparse
import subprocess
...
def make_env(seed=None, reward_scale=1.0, team_size=2, dynamic_kwargs=None, time_limit=10.0):
    """Erstelle ein einzelnes Environment mit Dynamic Scoring V2 Wrapper."""
    dynamic_kwargs = dynamic_kwargs or {}
    return make_env_with_dynamic_rewards_v2(
        seed=seed,
...
class ParallelEnvWorker:
    ...
    def __init__(self, worker_id, seed, reward_scale, team_size, dynamic_kwargs, time_limit):
...
def collect_rollout(workers, agent, device, rollout_steps, obs_dim_per_agent, deterministic=False):
    ...
    Rückgabe: (rollout_tensors, episode_stats)
      ...
      episode_stats: dict mit 'episode_rewards', 'episode_steps', 'branch_sums'
    """
...
def update_policy(agent, optimizer, rollout_data, args, update_step, writer, scaler=None):
    ...
    # Returns Normalisierung (laufender Mittelwert) mit NaN-Schutz
    current_mean = returns_t.mean().item()
    current_std = returns_t.std().item() + 1e-8
    
    # Schutz vor NaN/Inf in Value-Statistiken
    if not (np.isfinite(current_mean) and np.isfinite(current_std)):
        print(f"[WARN] Non-finite value stats (mean={current_mean}, std={current_std}), skipping normalization")
        returns_norm_t = returns_t
    else:
        if not hasattr(agent, '_value_mean') or not np.isfinite(agent._value_mean):
            agent._value_mean = current_mean
            agent._value_std = current_std
        else:
            alpha = 0.1
            agent._value_mean = (1 - alpha) * agent._value_mean + alpha * current_mean
            agent._value_std = (1 - alpha) * agent._value_std + alpha * current_std
        
        returns_norm_t = (returns_t - agent._value_mean) / agent._value_std
        returns_norm_t = torch.clamp(returns_norm_t, -10.0, 10.0)
...
def train(args):
    ...
    print(f"\nStarting ONLINE DYNAMIC SCORING V2 MAPPO training")
    ...
    avg100 = np.mean(episode_rewards[-100:]) if len(episode_rewards) >= 100 else np.mean(episode_rewards) if episode_rewards else 0.0
    print(f"\n{'='*60}")
    print(f"Online Dynamic Scoring V2 MAPPO Training finished!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=3e-4,
                        help="Learning rate (höher für online updates)")
    parser.add_argument("--lr-decay", type=float, default=0.9)
    parser.add_argument("--adam-eps", type=float, default=1e-4,
                        help="Adam epsilon - höher (1e-4) für mehr Stabilität, niedriger (1e-5) für schnellere Konvergenz")
    ...
    parser.add_argument("--max-grad-norm", type=float, default=0.3,
                        help="Gradient clipping norm - niedriger (0.3) für Stabilität, höher (0.5) für schnelleres Lernen")
    ...
    parser.add_argument("--log-dir", type=str, default="logs/soccer_mappo_dynamic_v2_online")
