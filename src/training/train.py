...
Neuer sauberer Start aus dem Archiv (vormals train_mappo_simple_ball.py).
Ziel: Symmetrisches, positives Reward-Shaping für Ball-Anlaufen in 2v2 Soccer.
"""
import os
import argparse
import subprocess
...
from src.env.soccer_env import make_env_with_simple_ball_chase
...
viewer.launch(env, policy=policy, title="MAPPO Simple Ball-Chase Soccer Agent")
'''
    path = '/tmp/soccer_mappo_simple_ball_viewer.py'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simple Ball-Chase MAPPO Training for Soccer (clean project start)",
