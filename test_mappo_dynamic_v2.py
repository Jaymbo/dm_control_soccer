"""
MAPPO Dynamic Scoring V2 Agent testen und visualisieren.

Testet einen mit Dynamic Scoring V2 Rewards trainierten MAPPO-Agenten.

Usage:
    python test_mappo_dynamic_v2.py                          # Neuesten Checkpoint laden
    python test_mappo_dynamic_v2.py --checkpoint path.pt     # Spezifischen Checkpoint laden
    python test_mappo_dynamic_v2.py --num-episodes 5         # Mehrere Episoden
    python test_mappo_dynamic_v2.py --time-limit 30.0        # Laengeres Spiel

Beispiele:
    # Lokal trainierten Agenten testen (2v2)
    python test_mappo_dynamic_v2.py --checkpoint logs/soccer_mappo_dynamic_v2/best_agent.pt --viewer

    # 3v3 mit laengerer Spieldauer
    python test_mappo_dynamic_v2.py --team-size 3 --time-limit 30.0 --viewer

    # Besten Checkpoint aus Optuna-Study laden
    python test_mappo_dynamic_v2.py --log-dir logs/optuna_dynamic_v2/trial_5
"""
import argparse
import os
import glob
...
def find_latest_checkpoint(log_dir="logs/soccer_mappo_dynamic_v2"):
...
def test(args):
    ...
    print("\nDynamic Scoring V2 Agent - trained with:")
    print("  - Recovery:   Ball-Chaser laeuft zum Ball")
    print("  - Marking:    Gegner decken / Angriffsposition")
    print("  - Possession: Ball zum Tor bringen")
    print("  - Attack Pos: Freie Torsicht belohnt")
    print("  - Shooting:   Schuss aufs Tor")
    print("  - Blocking:   Schuss abwehren")
    print("  - Goalkeeping: Tor-Positionierung")
    
    try:
        viewer.launch(env, policy=policy, title=f"MAPPO Dynamic V2 Soccer ({args.team_size}v{args.team_size})")
